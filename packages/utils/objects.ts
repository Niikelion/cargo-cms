...
export const mapRecordEntries = <Key extends string | symbol, Value extends any, Result extends any>(source: Record<Key, Value>, map: (v: Value, k: Key) => [Key, Result]): Record<Key, Result> =>
    (Object.fromEntries(Object.entries<Value>(source).map(([key, value]) => map(value as Value, key as Key))) as Record<Key, Result>)
